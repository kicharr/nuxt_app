...
import ChangeTaskModal from "~/components/planner/ChangeTaskModal.vue";
import {stringsLengthHandler} from "~/utils/utils.js";

const props = defineProps(['category', 'item']);
...
const openSettings = () => {
  showSettingsDrop.value = !showSettingsDrop.value
}
const isModalOpen = () => modalShow.value = !modalShow.value;
const onDragStart = (e, item) => emit('onDragStart', e, item);
const deleteTaskItem = (id) => {
  plannerStore.deleteTask(id);
  openSettings();
}
const changeTask = (changedData) => {
  plannerStore.updateTask(changedData.id, changedData);
}

</script>

<template>
  <ChangeTaskModal v-if="modalShow" @closeModal="isModalOpen" @openSettings="openSettings" @changeTask="changeTask"
